...
import CategoriesTableRow from "@/views/CategoriesView/CategoriesTableRow.vue";
export default {
  components: { TableComponent, CategoriesTableRow },
  ...
  methods: {
    async onSubmit() {
      ...
      const foundedIndex = this.categories.findIndex(
        (c) => c.id === newForm.id
      );
      let response;
      if (this.form.id !== null) {
        ...
        this.categories.splice(foundedIndex, 1, response.data);
        this.clearForm();
      } else {
    ...
    async deleteItem(id) {
      try {
        ...
        // this.categories.splice(foundedIndex, id);
        this.categories = this.categories.filter(
          (c) => c.id !== deletedItem.data.id
        );
      } catch (error) {}
    },
  },
  ...
  async mounted() {
    try {
      ...
      this.categories = categories.data.reverse();
    } catch (error) {
      console.error(error);
    }
  },
